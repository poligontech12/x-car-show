...
import { decodeUploadedImage } from '../lib/image';
...
test('spotted photos are decoded and normalized to a bounded JPEG', async () => {
  const photo = await decodeUploadedImage(`data:image/png;base64,${PIXEL_PNG}`);
...
test('spotted photos reject unsupported, malformed, and oversized payloads', async () => {
  await expect(decodeUploadedImage('data:image/svg+xml;base64,PHN2Zy8+')).rejects.toThrow(
    'Alege o fotografie JPEG, PNG sau WebP.',
  );
  await expect(decodeUploadedImage('data:image/jpeg;base64,/9j/2Q==')).rejects.toThrow(
    'Fotografia nu poate fi citită.',
  );
  await expect(decodeUploadedImage(`data:image/jpeg;base64,${PIXEL_PNG}`)).rejects.toThrow(
    'Fotografia nu poate fi citită.',
  );

  const oversized = `data:image/jpeg;base64,${Buffer.alloc(1_000_001).toString('base64')}`;
  await expect(decodeUploadedImage(oversized)).rejects.toThrow('Fotografia este prea mare.');
});
